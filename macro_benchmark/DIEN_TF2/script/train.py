...
parser.add_argument("--embedding_device", type=str, default='gpu',
...
def eval(sess, test_data, model, model_path):
    ...
    for src, tgt in test_data:
        ...
        prob, loss, acc, aux_loss = model.calculate(
            sess, [uids, mids, cats, mid_his, cat_his, mid_mask, target, sl, noclk_mids, noclk_cats])
        end_time = time.time()
...
def train_synthetic(
    ...
    sess_config = tf.compat.v1.ConfigProto(gpu_options=gpu_options, log_device_placement=False)
    ...
    with tf.compat.v1.Session(config=sess_config) as sess:
        ...
        if model_type == 'DNN':
            ...
            model = Model_DIN_V2_Gru_Vec_attGru_Neg(
                n_uid, n_mid, n_cat, EMBEDDING_DIM, HIDDEN_SIZE, ATTENTION_SIZE, data_type, device=embedding_device)
        else:
            print("Invalid model_type : %s", model_type)
            return
        # for var in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES):
        ...
        approximate_accelerator_time = 0
        test_auc, test_loss, test_accuracy, test_aux_loss, eval_time, num_iters = eval(
            sess, test_data, model, model_path)
        approximate_accelerator_time += eval_time
        print('test_auc: %.4f ----test_loss: %.4f ---- test_accuracy: %.9f ---- test_aux_loss: %.4f ---- eval_time: %.3f' %
              (test_auc, test_loss, test_accuracy, test_aux_loss, eval_time))
        test_auc, test_loss, test_accuracy, test_aux_loss, eval_time, num_iters = eval(
            sess, test_data, model, model_path)
        approximate_accelerator_time += eval_time
        print('test_auc: %.4f ----test_loss: %.4f ---- test_accuracy: %.9f ---- test_aux_loss: %.4f ---- eval_time: %.3f' %
...
if __name__ == '__main__':
    ...
    random.seed(SEED)
    if args.mode == 'train':
        train(model_type=args.model, seed=SEED, batch_size=args.batch_size,
              data_type=args.data_type, embedding_device=args.embedding_device)
    elif args.mode == 'test':
        test(model_type=args.model, seed=SEED, batch_size=args.batch_size,
             data_type=args.data_type, embedding_device=args.embedding_device)
